...
* You can either specify options on the command-line, or in a *config
  ...
  `traitlets.config <https://traitlets.readthedocs.io/en/stable/config.html>`_).
  Specify this configuration file using the ``--config=FILENAME`` option.
...
7. add the file and ``main()`` function you made in step 6 to the
   ...
   an executable called ``ctapipe-*`` where ``*`` is the name of the source
